...
printf '\n  2) \033[1mComfy Registry —— 已自动化,别再手动 publish\033[0m\n'
printf '     .github/workflows/publish.yml:push main 且 pyproject.toml 有变更 → 自动发布。\n'
printf '     \033[1m所以上面那个 push 本身就是发布动作,不是保存动作。\033[0m\n'
printf '     手动再跑一次 comfy node publish 会撞「版本已存在」(Registry 版本不可变)。\n'
printf '     若 pyproject.toml 这次没变(如仅补发),去 Actions 手动 Run workflow。\n'
printf '     一次性配置见 RELEASING.md(需仓库 secret REGISTRY_ACCESS_TOKEN)。\n'
