...
that re-POSTs the CURRENT known progress to the relay every
JOB_HEARTBEAT_INTERVAL, so the relay's updated_at keeps advancing and its reaper
never mistakes a long-but-healthy generation for a dead plugin. These tests pin:

  * the heartbeat fires while a job is running (even with no new progress);
  * it re-sends the latest known (value, max), not stale 0s;
...
class TestJobHeartbeat(unittest.IsolatedAsyncioTestCase):
    async def test_fires_during_a_quiet_job_and_stops_after(self):
        ...
        async def fake_run_prompt(job_id, prompt):
            w._progress = (7, 20)  # the job has seen some progress
            started.set()
            await release.wait()
            return "pid-1"
        ...
        with mock.patch.object(worker, "JOB_HEARTBEAT_INTERVAL", 0.02), \
            ...
            # Heartbeats carry the CURRENT known progress, not stale 0/0.
            self.assertTrue(
                any(c.args[1:] == ("running", 7, 20) for c in relay.progress.call_args_list),
                f"heartbeat should re-send (7, 20); calls={relay.progress.call_args_list}",
            )
